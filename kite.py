...
REFRESH_INTERVAL = int(os.getenv('REFRESH_INTERVAL', '10'))  # Increased to 10s to avoid rate limits
...
class TechnicalAnalyzer:
    ...
    def compute_vwap(candles: List[Dict]) -> Tuple[Optional[float], Optional[float]]:
        ...
        for c in candles:
            close = c.get('close')
            volume = c.get('volume', 0)  # Default to 0 if missing
            ...
            try:
                close = float(close)
                volume = float(volume) if volume else 1.0  # Use 1.0 if volume is 0
    ...
    def compute_ema(values: List[float], period: int) -> Optional[float]:
        ...
        # Use all available values, no minimum requirement
        k = 2.0 / (period + 1.0)
        ema_val = None
    ...
    def compute_rsi(values: List[float], period: int = 14) -> Optional[float]:
        ...
        avg_gain = sum(gains[:use_period]) / use_period
        avg_loss = sum(losses[:use_period]) / use_period
        
        for i in range(use_period, len(gains)):
            avg_gain = (avg_gain * (use_period - 1) + gains[i]) / use_period
            avg_loss = (avg_loss * (use_period - 1) + losses[i]) / use_period
    ...
    def compute_macd(values: List[float], fast: int = 12, slow: int = 26, signal: int = 9) -> Tuple[Optional[float], Optional[float]]:
        ...
        signal_line = macd * 0.9  # Approximation when we don't have history
    ...
    def compute_bollinger_bands(values: List[float], period: int = 20, std_dev: float = 2.0) -> Tuple[Optional[float], Optional[float], Optional[float]]:
        ...
        # Use available data even if less than period
        use_period = min(period, len(values))
        sma = TechnicalAnalyzer.compute_sma(values, use_period)
        if sma is None:
            return None, None, None
            
        recent = values[-use_period:]
        variance = sum((x - sma) ** 2 for x in recent) / use_period
        std = variance ** 0.5
    ...
    def compute_adx(candles: List[Dict], period: int = 14) -> Optional[float]:
        ...
        # Use available data even if less than period
        use_period = min(period, len(plus_dm))
        plus_di = (sum(plus_dm[-use_period:]) / use_period / atr) * 100
        minus_di = (sum(minus_dm[-use_period:]) / use_period / atr) * 100
...
class KiteDataFetcher:
    ...
    def fetch_intraday_data(self) -> Tuple[List[Dict], List[float], Optional[float]]:
        ...
        try:
            ...
            logger.info(f"Fetching intraday data from {start_dt} to {end_dt}")
            ...
            logger.info(f"Received {len(data) if data else 0} minute candles")
            ...
            logger.info(f"Processed {len(candles)} valid candles, last price: {last_price}")
    ...
    def fetch_option_chain(self):
        ...
        try:
            ...
            logger.info(f"Intraday data: {len(intraday_candles)} candles, {len(intraday_closes)} closes")
            
            if last_price:
                spot = last_price
            
            intraday_indicators = TechnicalAnalyzer.analyze_full(intraday_candles, intraday_closes)
            logger.info(f"Intraday indicators computed: VWAP={intraday_indicators.vwap}, EMA9={intraday_indicators.ema9}, RSI={intraday_indicators.rsi14}")
            ...
            # Build option records with rate limiting
            def build_records(expiry):
                opts = [i for i in nifty_options if i['expiry'] == expiry]
                strikes = sorted(list(set([i['strike'] for i in opts])))
                
                # Filter strikes around spot (±500 points)
                if spot:
                    strikes = [s for s in strikes if abs(s - spot) <= 500]
                
                records = []
                
                # Batch quote requests to avoid rate limits
                all_tokens = []
                strike_token_map = {}
                ...
                for strike in strikes:
                    ...
                    strike_token_map[strike] = {'ce': ce_token, 'pe': pe_token}
                ...
                # Fetch quotes in batches of 100 (Kite API limit)
                quote_data = {}
                batch_size = 100
                ...
                for strike in strikes:
                    tokens = strike_token_map[strike]
                    ce_token = tokens['ce']
                    pe_token = tokens['pe']
            ...
            logger.info("Fetching weekly option chain...")
            weekly_records = build_records(nearest_weekly)
            logger.info(f"Fetched {len(weekly_records)} weekly records")
            
            time.sleep(0.5)  # Delay between weekly and monthly
            
            logger.info("Fetching monthly option chain...")
            monthly_records = build_records(nearest_monthly)
            logger.info(f"Fetched {len(monthly_records)} monthly records")
...
class HTMLRenderer:
    ...
    def render_html(data: Dict, output_path: str = "option_chain_v2.html", open_browser: bool = True):
        ...
        logger.info(f"Indicators - VWAP: {has_vwap}, EMA: {has_ema}, RSI: {has_rsi}, MACD: {has_macd}, Count: {ind_count}")
